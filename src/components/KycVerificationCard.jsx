...
const KycVerificationCard = () => {
  ...
  const handleAadharSubmit = async (e) => {
    ...
    try {
      await api.post('/kyc/send-otp', formData, {
  ...
  const handlePanSubmit = async (e) => {
    ...
    try {
      await api.post('/kyc/pan', formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      toast.success('PAN card verified! KYC complete.');
      qc.invalidateQueries({ queryKey: ['wallet-balance'] });
      setStep(4);
    } catch (err) {
  ...
  return (
    <div className="w-full bg-card border border-border rounded-3xl p-6 relative overflow-hidden mb-6 shadow-xl">
      ...
      <div className="grid grid-cols-4 gap-2 mb-6">
        {[
          { label: 'Aadhaar', active: step === 1 || step === 2, done: step > 2 || step === 'done' },
          { label: 'Verify OTP', active: step === 2, done: step > 2 || step === 'done' },
      ...
      <div className="bg-background/40 border border-border/60 rounded-2xl p-5 relative z-10">
        ...
        {step === 1 && termsAgreed && (
          <form onSubmit={handleAadharSubmit} className="space-y-4">
            <div className="grid md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider block mb-1.5">Aadhaar Photo / Document (Optional)</label>
            ...
            >
              {loading ? <Loader2 size={14} className="animate-spin" /> : <>Get Verification OTP <ArrowRight size={14} /></>}
            </button>
          </form>
        )}
        ...
        {step === 2 && (
          <form onSubmit={handleOtpSubmit} className="space-y-4">
            <div>
              <label className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider block mb-1.5">Enter 6-Digit OTP</label>
              <p className="text-[11px] text-muted-foreground mb-2">A mock OTP has been logged to the console for testing.</p>
              <div className="flex bg-accent/40 border border-border rounded-xl px-3.5 py-3 items-center focus-within:border-primary/40 transition-all max-w-xs">
                ...
                <input 
                  ...
                  className="bg-transparent border-none outline-none w-full text-sm font-mono font-bold text-foreground tracking-widest placeholder:tracking-normal placeholder:font-sans" 
                  required
                />
              </div>
            </div>
            <div className="flex gap-2">
              <button 
                type="button" 
                onClick={() => setStep(1)} 
                className="px-5 py-3 border border-border rounded-xl text-muted-foreground font-semibold text-xs transition-colors hover:border-muted-foreground"
              >
                Back
              </button>
              <button 
                ...
                className="px-6 py-3 bg-emerald-600 text-white font-bold text-xs rounded-xl hover:opacity-90 transition-all flex items-center justify-center gap-2 disabled:opacity-40"
              >
                {loading ? <Loader2 size={14} className="animate-spin" /> : <>Verify Aadhaar <CheckCircle2 size={14} /></>}
              </button>
            </div>
          </form>
        )}
        ...
        {step === 3 && (
          <form onSubmit={handlePanSubmit} className="space-y-4">
            ...
            <button 
              ...
              className="w-full sm:w-auto px-6 py-3 bg-primary text-primary-foreground font-bold text-xs rounded-xl shadow-lg hover:opacity-90 transition-all flex items-center justify-center gap-2 disabled:opacity-40"
            >
              {loading ? <Loader2 size={14} className="animate-spin" /> : <>Verify PAN Card <CheckCircle2 size={14} /></>}
            </button>
          </form>
        )}
          ...
          <div className="bg-card border border-border rounded-3xl p-6 md:p-8 w-full max-w-lg shadow-2xl relative flex flex-col max-h-[85vh] animate-in zoom-in-95 duration-200">
            ...
            <label className={`flex items-start gap-2.5 mt-5 p-3 rounded-xl border transition-all ${
              ...
              <div className="text-[11px] font-semibold select-none leading-relaxed">
                I agree to the Terms & Conditions of SkillSync. I verify that all documents submitted belong to me.
